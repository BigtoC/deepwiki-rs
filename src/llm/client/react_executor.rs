...
use anyhow::Result;
use rig::{
    completion::{PromptError, Message, Prompt},
    providers::mistral,
};
...
impl ReActExecutor {
    ...
    pub async fn execute(
        agent: &rig::agent::Agent<mistral::CompletionModel>,
        user_prompt: &str,
        config: &ReActConfig,
    ) -> Result<ReActResponse> {
        if config.verbose {
            println!("🤖 开始ReAct模式对话，最大迭代次数: {}", config.max_iterations);
        }
            ...
            .await
        {
            ...
            Err(PromptError::MaxDepthError { max_depth, chat_history, prompt: _ }) => {
                if config.verbose {
    ...
    fn extract_partial_result(chat_history: &[Message]) -> (String, Vec<String>) {
        ...
        let last_assistant_message = chat_history
            ...
            .unwrap_or_else(|| {
                "对话因达到最大迭代次数而被截断，未能获得完整响应。".to_string()
            });
